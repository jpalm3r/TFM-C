...
int main(){
  ...
  *********************************************************************/
  ...
  int j,r,rep;
  int RUNS = 1; int REPS = 9; int T = 500;
  ...
  float sigma;
  float SumTotalRate;
  float lambda, lambda0 = 0.1, MAXlambda = 0.8, delta_lambda = 0.1; // MAXlambda; // rate of activation
  float lambda_1c;

  // if(strcmp(,"pgp") == 0){ // checking what net is studied, values from previous analysis
  //   lambda_1c = 0.03;
  // }else if(strcmp(,"bkite") == 0){
  //   lambda_1c = 0.05;
  // }else{
  //   lambda_1c = 0.04;
  // }

  float eta = 1; // rate of recovery
  float mu = 0.08; // rate of spontaneous activation
  struct network* LAYERS;
  struct network* LAYERS_0;
  //struct competition Prob;
  float* G_COEFFS;
  int lenss; int bkt = 5; int tstep;
  //char* fileTest;
  ...
  */
  ...
  printf("      Parameters:\n");
  printf("\n");
  // printf("        - lambda_0/lambda_1c = %f\n", lambda0/(float)lambda_1c);
  printf("        - sigma  = %f\n", sigma);
  printf("\n");
  printf("        - epsilon_0  = %f\n", epsilon);
  printf("      -------------------------------\n");
  ...
  LambdaFile = fopen(pathFile,"w");
  epsilon = epsilon0; lambda = lambda0;

  for(rep=0;rep<REPS;rep++){ ///////////////////////////////////////////////////

    sprintf(LambdaDir, "./%s/lambda-%f",EvolutionDir,lambda); if(stat(LambdaDir, &dir) == -1){mkdir(LambdaDir, 0755);}

    for(r=1;r<=RUNS;r++){ //////////////////////////////////////////////////////

      epsilon = epsilon0;
      sprintf(pathFile, "./%s/%d.txt",LambdaDir,r);
      Evolution_File = fopen(pathFile,"w");
      WriteHeader(Evolution_File,lambda,epsilon,sigma,FRAC[0],FRAC[1]);

      ResetVectors(LAYERS,LAYERS_0,cl);
      Compute_Density(LAYERS, cl);
      Update_Weight(LAYERS, cl, epsilon0, sigma);
      ...
      while((keep_running)){
        ...
        Compute_GllspCoeffs(LAYERS, cl, eta, G_COEFFS);
        ...
        Compute_Density(LAYERS, cl);
        Update_Weight(LAYERS, cl, epsilon, sigma);
        Update_ActRate(LAYERS, cl, lambda, mu);
	  ...
	  keep_running = !CoexDied(LAYERS,cl);
         }

      }// END OF GILLESPIE /////////////////////////////////////////////////////

    fprintf(LambdaFile,"%f %f\n",lambda,epsilon);
