...
void Event_Happens(struct network *LAYERS, float inf, float* COEFFS){ //ONLY WORKING FOR 2 NETS
  ...
  *********************************************************************************/
    ...
    else if (((COEFFS[0] < inf) && (inf <= COEFFS[1])) && ((LAYERS[0].SIS.Nact > 0)&&(LAYERS[0].SIS.Eact > 0))){ // deactivation
    ...
    else if (((COEFFS[4] < inf) && (inf <= COEFFS[5])) && ((LAYERS[1].SIS.Nact > 0)&&(LAYERS[1].SIS.Eact > 0))){ // deactivation
    ...
    // DebugPointers(&LAYERS[0]);
    // DebugPointers(&LAYERS[1]);
...
void Compute_GllspCoeffs(struct network *LAYERS,int cl, float sigma, float* COEFFS){
  ...
  for (i=0;i<cl;i++){
    ...
    LAYERS[i].SIS.a_i.mmact = LAYERS[i].mu*LAYERS[i].SIS.Nsus/(float)A_0;
...
void Update_ActRate(struct network *LAYERS, int cl, float lambda, float mu){
  ...
  for(i=0;i<cl;i++){
    ...
    LAYERS[i].mu = mu*LAYERS[i].weight;
...
bool CoexDied(struct network *LAYERS, int cl){
  ...
  dead = false;
  i=0;
  //IMPORTANT!!! Diferent from other functions.c, here only need
  // one epidemic to end
  while((i<cl)&& !dead){
    // dead = (LAYERS[i].rho.act < 0.00005);
    dead = (LAYERS[i].rho.act < 1/(float)LAYERS[i].EN.N);
    i += 1;
  }
...
bool Coex(struct network *LAYERS){
  ...
  return (LAYERS[0].rho.act > 0.005) && (LAYERS[1].rho.act > 0.005);
...
bool StationaryState(struct network *LAYERS, int cl, int bkt){
  ...
  for(j=0;j<cl;j++){
    ...
    while ((i<bkt) && (SS[j])){
      SS[j] = (abs(LAYERS[j].SIS.Tss[i] - LAYERS[j].SIS.Tss[0]) <= 0.1*(float)LAYERS[j].SIS.Tss[0]); // 5% tolerance
      i += 1;
    }
    sstate += SS[j];

    for(i=0;i<bkt;i++){ //reseting Tss
      LAYERS[j].SIS.Tss[i] = 0;
    }
  }
...
void speedUpdateDeactivation(struct network *net, int j, int recovered){
  ...
  // printf("--------------------------------------------\n");
  // printf("  pEA_S : ");
  // for(k=0;k<net->SIS.count.s2a;k++){
  //   printf("%d ", net->SIS.pointEA_S[k]);
  // }printf("\n");
  // printf("  ppS : ");
  // for(k=0;k<net->lenLINKS;k++){
  //   if(net->SIS.ppS[k] != 0){printf("%d ", net->SIS.ppS[k]);}
  // }printf("\n");
  // printf("  pEA_P : ");
  // for(k=0;k<net->SIS.count.p2a;k++){
  //   printf("%d ", net->SIS.pointEA_P[k]);
  // }printf("\n");
  // printf("  ppP : ");
  // for(k=0;k<net->lenLINKS;k++){
  //   if(net->SIS.ppP[k] != 0){printf("%d ", net->SIS.ppP[k]);}
  // }printf("--------------------------------------------\n");
